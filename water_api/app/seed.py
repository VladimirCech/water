...
from app.models import Base, User, Game, Build, Entitlement
...
def main():
    ...
    with session_scope() as s:
        ...
        if not b:
            ...
            s.add(b); s.flush()
        e = s.execute(select(Entitlement).where(Entitlement.user_id==u.id, Entitlement.game_id==g.id)).scalar_one_or_none()
        if not e:
